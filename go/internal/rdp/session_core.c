...
#define PROGRESSIVE_MIN_SEND_INTERVAL_MS 16U
...
static void session_write_u16(uint8_t* data, uint16_t value)
{
...
{
	if (!server->hooks.send_control)
		return false;
	return server->hooks.send_control(server->hooks.context, type, payload, length);
}
...
static bool server_set_stream_requested(Server* server, bool requested)
{
	...
	const uint8_t type = requested ? 2 : 3;
	const bool sent = server_send_control(server, type, NULL, 0);
...
static BOOL on_mouse(rdpInput* input, UINT16 flags, UINT16 x, UINT16 y)
{
	...
	session_write_u16(payload, hid_clamp_absolute(x, width));
	session_write_u16(payload + 2, hid_clamp_absolute(y, height));
	session_write_u16(payload + 4, width);
	session_write_u16(payload + 6, height);
	session_write_u16(payload + 8, flags);
...
static BOOL on_relative_mouse(rdpInput* input, UINT16 flags, INT16 x_delta, INT16 y_delta)
{
	...
	uint8_t payload[5] = { 0 };
	session_write_u16(payload, (uint16_t)x_delta);
	session_write_u16(payload + 2, (uint16_t)y_delta);
	...
	session_write_u16(wheel_payload, flags);
...
static DWORD WINAPI peer_thread(LPVOID argument)
{
	...
	while (!client_should_stop(client))
	{
		...
		if (client->bitmap_fallback_active)
		{
			...
			{
				const uint64_t now = monotonic_milliseconds();
				if (last_send_started_at == 0 || now - last_send_started_at >= interval)
					timeout = 0;
				else
					timeout = WINPR_ASSERTING_INT_CAST(DWORD, interval - (now - last_send_started_at));
			}
		}
...
static bool session_read_h264(Server* server, uint8_t** data, size_t* length, uint32_t* losses)
{
	if (!server->hooks.read_h264)
		return false;
	return server->hooks.read_h264(server->hooks.context, data, length, losses);
}
...
static DWORD WINAPI video_thread(LPVOID argument)
{
	...
	while (!client_should_stop(client))
	{
		uint8_t* data = NULL;
		size_t length = 0;
		uint32_t losses = observed_losses;
		if (!session_read_h264(client->server, &data, &length, &losses))
		{
...
static DWORD WINAPI bitmap_video_thread(LPVOID argument)
{
	...
	const uint16_t width = client->render_width;
	const uint16_t height = client->render_height;
	if (!client->server->hooks.decode_start ||
	    !client->server->hooks.decode_start(client->server->hooks.context, width, height))
	{
		log_message("ERROR", "FFmpeg decoder를 시작할 수 없습니다");
		client_stop(client);
		return 0;
	}
	uint32_t observed_losses = 0;
	while (!client_should_stop(client))
	{
		...
		const bool pushed = client->server->hooks.decode_push &&
		                    client->server->hooks.decode_push(client->server->hooks.context, annexb, annexb_length);
		free(annexb);
		free(data);
		if (!pushed)
		{
	...
	if (client->server->hooks.decode_stop)
		client->server->hooks.decode_stop(client->server->hooks.context);
	return 0;
}


struct NanokvmRdpSession
{
	Server server;
	freerdp_listener* listener;
	volatile sig_atomic_t local_stop;
};

static void session_signal(int signal_number)
{
	(void)signal_number;
	stop_requested = 1;
}
...
NanokvmRdpSession* nanokvm_session_start(const NanokvmRdpConfig* config, const NanokvmSessionHooks* hooks)
{
	...
	session->server.config.swap_alt_command = config->swap_alt_command;
	...
	if (!InitializeCriticalSectionAndSpinCount(&session->server.lock, 4000) ||
		...
		goto fail;
	session->listener = freerdp_listener_new();
	if (!session->listener)
		goto fail;
	session->listener->info = &session->server;
	session->listener->PeerAccepted = peer_accepted;
	if (!session->listener->Open(session->listener, config->bind_address, config->port))
		goto fail;
	(void)signal(SIGINT, session_signal);
	(void)signal(SIGTERM, session_signal);
	(void)signal(SIGPIPE, SIG_IGN);
	return session;
fail:
	nanokvm_session_stop(session);
	return NULL;
}
...
bool nanokvm_session_pump(NanokvmRdpSession* session, uint32_t timeout_ms)
{
	...
	const DWORD count = session->listener->GetEventHandles(session->listener, handles, ARRAYSIZE(handles));
	if (count == 0 || WaitForMultipleObjects(count, handles, FALSE, timeout_ms) == WAIT_FAILED)
		return false;
	return session->listener->CheckFileDescriptor(session->listener) == TRUE;
}


bool nanokvm_session_submit_bitmap(NanokvmRdpSession* session, const uint8_t* bgra, size_t length)
{
	if (!session || !bgra || length == 0)
		return false;
	Client* client = NULL;
	EnterCriticalSection(&session->server.lock);
	client = session->server.active;
	LeaveCriticalSection(&session->server.lock);
	if (!client)
		return false;
	return on_decoded_bitmap_frame(client, bgra, length);
}
...
void nanokvm_session_stop(NanokvmRdpSession* session)
{
	...
	if (session->listener)
	{
		if (session->listener->Close)
			session->listener->Close(session->listener);
		freerdp_listener_free(session->listener);
	}
